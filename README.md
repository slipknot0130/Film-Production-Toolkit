...
#### 🎯 分镜目标时长（密度）控制（v4.6 新增）

在分镜工作台左侧栏新增「🎯 分镜目标时长（密度）」子选项，让同一剧本按不同目标成片时长拆出不同密度的镜头：

| 选项 | 目标单集时长 | 适用 |
|------|:---:|------|
| 自动（按剧本字数密度） | 由字数推算（约 1 镜/24 字） | 让程序按剧本体量自适应 |
| 短剧 · 2 分钟/集 | 2 分钟 | 短剧：镜头少而长 |
| 竖屏短剧 · 3 分钟/集 | 3 分钟 | 竖屏微短剧 |
| 标准 · 10 分钟/集 | 10 分钟 | 常规网剧单集 |
| 长剧单集 · 45 分钟/集 | 45 分钟 | 长剧：镜头多而密 |
| 自定义 | 0.5–180 分钟 | 任意目标时长 |

**工作原理：**
- 选中目标后，程序按 `目标秒 ÷ 平均镜秒`（平均镜秒为固定参考假设：对白多 4.5s / 均衡 5.0s / 动作 4.0s）反推应产出的**镜头数**，作为 Director 的硬性下限要求，禁止少拆。
- 分块模式下，各块按**字符占比**分摊目标时长，保证长剧本整体密度达标；单块镜数上限由**所选模型的输出上限动态推算**（如 16K → 40 镜/块，8K → 20 镜/块），从源头杜绝输出被截断。
- **参考时长 ≠ 强制时长**：程序只输出参考时长与对应镜头数；每个镜头在即梦(Seedance)里的实际长度由你自行设置。短剧本（如 ~5000 字）在 45 分钟档下最多约 252 镜（≈17 分钟参考），UI 会提示「剧本太短」，不会假装达到目标时长——此时可在即梦里拉长单镜来补足成片长度。

**QA 兜底：** 质检 Agent 新增「镜头数下限审查」，若实际产出低于硬性下限则判为不合格，要求重拆。
