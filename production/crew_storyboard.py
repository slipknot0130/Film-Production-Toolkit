...
# 历史 bug（5963d44 引入）：为防截断把 max_tokens 提到 16000，结果旧版 DeepSeek
# （其时上限 8192）直接拒绝请求，三次重试全挂 → 整篇分镜零输出。
#
# 2026-09：DeepSeek 输出上限已提升到 384K，shared 层的实用预算取 16384
#   （≈ 单块 40 镜）。分镜侧长期请求的 16000 现已能正常通过、不再被钳死，
#   长剧本的人物/场景全篇提取也不会再因截断而解析失败。
#
...
你只需专注输出每个叙事节拍的完整画面内容、台词和主导景别。
代码层会在后续自动为每个镜头补充：
  · 远景/全景建立 → 中景/过肩对话 → 特写/近景强调 的机位序列
  · 每个镜头的 [镜头时间轴]，说明 0-X秒、X-Y秒、Y-Z秒 的机位变化与运动
  · 时长 ≥8秒 的丰富镜头会进一步拆分为 2-3 个独立子镜头，每个子镜头独立机位

因此你不需要在单个「画面内容」中堆砌多个角度，只要把当前节拍的动作、台词、情绪写足写细即可。
...
N. 子镜头机位丰富度审查：
   - 每个镜头的「画面内容」是否包含足够的动作/台词/表情/道具细节，能被代码层扩展为远景→中景→特写等多角度序列？
   - 若画面内容过于干瘪（仅一句动作），补充细节以便生成丰富子镜头。
   - 长对话若被压缩在单镜，是否可按对白轮次拆到多个子镜头？
...
def _result_to_shots(result, time_offset_seconds):
    ...
    # v5.0：镜头扩展与机位丰富化——把长镜头拆分为多机位子镜头，
    # 短镜头则注入 [镜头时间轴] 实现单镜头内多角度变化。
    try:
        from production.shot_expander import expand_shots_in_data
        shot_data = expand_shots_in_data(shot_data)
    except Exception as e:
        logger.warning(f"[_result_to_shots] 镜头扩展失败，回退到原输出: {type(e).__name__}: {e}")
